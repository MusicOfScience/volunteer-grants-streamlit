...
if "confirmed_settings" not in st.session_state:
    st.session_state.confirmed_settings = {
...
st.caption("Volunteer Grants only · budget lock + revert controls · committee-friendly recommendation views")

with st.sidebar:
    st.header("Model settings")
    st.subheader("Budget")
    pending_budget = st.number_input("Budget input", min_value=0.0, value=float(st.session_state.pending_budget), step=1000.0, key="budget_input_field")
    ...
    protected_threshold = st.number_input("Protected threshold", min_value=1000.0, value=float(st.session_state.confirmed_settings["protected_threshold"]), step=50.0)
    min_application = st.number_input("Minimum application", min_value=0.0, value=float(st.session_state.confirmed_settings["min_application"]), step=50.0)
    haircut_mode = st.selectbox("Haircut mode", ["percentage", "cap"], index=["percentage", "cap"].index(st.session_state.confirmed_settings["haircut_mode"]))
    haircut_rate = st.slider("Haircut rate", 0.0, 0.5, float(st.session_state.confirmed_settings["haircut_rate"]), 0.01)
    soft_cap = st.number_input("Soft cap", min_value=1000.0, value=float(st.session_state.confirmed_settings["soft_cap"]), step=100.0)
    penalty_weight = st.slider("Penalty weight", 0.0, 1.0, float(st.session_state.confirmed_settings["penalty_weight"]), 0.01)
    ...
    year_weight_2023_24 = st.slider("2023–24 weight", 0.0, 1.0, float(st.session_state.confirmed_settings["year_weight_2023_24"]), 0.01)
    year_weight_2024_25 = st.slider("2024–25 weight", 0.0, 1.0, float(st.session_state.confirmed_settings["year_weight_2024_25"]), 0.01)
    round_to_dollar = st.checkbox("Round allocations to whole dollars", value=bool(st.session_state.confirmed_settings["round_to_dollar"]))
    c3, c4 = st.columns(2)
    with c3:
        if st.button("Confirm other settings", use_container_width=True):
            st.session_state.confirmed_settings.update({
...
params = ModelParams(
    ...
    min_application=st.session_state.confirmed_settings["min_application"],
    protected_threshold=st.session_state.confirmed_settings["protected_threshold"],
    haircut_mode=st.session_state.confirmed_settings["haircut_mode"],
    haircut_rate=st.session_state.confirmed_settings["haircut_rate"],
    soft_cap=st.session_state.confirmed_settings["soft_cap"],
    penalty_weight=st.session_state.confirmed_settings["penalty_weight"],
    year_weight_2023_24=st.session_state.confirmed_settings["year_weight_2023_24"],
    year_weight_2024_25=st.session_state.confirmed_settings["year_weight_2024_25"],
    round_to_dollar=st.session_state.confirmed_settings["round_to_dollar"],
)
...
with tabs[6]:
    st.download_button("Download XLSX export", data=outputs["excel_bytes"], file_name=f"Volunteer_Grants_Allocation_Model_{APP_VERSION}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    st.download_button("Download allocation results CSV", data=outputs["results"].to_csv(index=False).encode("utf-8"), file_name=f"Volunteer_Grants_Allocation_Model_{APP_VERSION}.csv", mime="text/csv")
